...
async function load(): Promise<void> {
  ...
  try {
    ...
    operations.value = loadedOperations;
  } catch (error) { notify(messageFor(error), "error"); }
  finally { loading.value = false; }
}
const displayOperations = computed(() => operations.value.map((item) => item.status === "running" ? { ...item, status: "pending" as const } : item));
onMounted(load);
